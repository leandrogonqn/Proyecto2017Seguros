...
@DomainService(nature = NatureOfService.DOMAIN, repositoryFor = Poliza.class)
public class PolizaRepository {
	...
	public List<Poliza> buscarpolizaNumero(final String polizaNumero) {
		return repositoryService.allMatches(
				new QueryDefault<>(Poliza.class, "buscarPorNumeroPoliza", "polizaNumero", polizaNumero.toLowerCase()));
	}

	public List<Poliza> listarPorEstado(final Estado polizaEstado) {
		return repositoryService
				.allMatches(new QueryDefault<>(Poliza.class, "listarPorEstado", "polizaEstado", polizaEstado));
	}

	public List<Poliza> buscarPorCliente(final Persona polizaCliente) {
		return repositoryService
				.allMatches(new QueryDefault<>(Poliza.class, "buscarPorCliente", "polizaCliente", polizaCliente));
	}

	// Este metodo lista las polizas vigentes que aun no tienen renovacion
	// ordenadas por fecha de vencimiento (de la que falta menos a la que falta
	// mas)
	...
	public List<Poliza> buscarPorCompania(final Compania polizaCompania) {
		return repositoryService
				.allMatches(new QueryDefault<>(Poliza.class, "buscarPorCompania", "polizaCompania", polizaCompania));
	}
