...
@DomainService(
        ...
        repositoryFor = RiesgosAP.class
)
public class RiesgosAPRepository {

    public List<RiesgosAP> listar() {
        return repositoryService.allInstances(RiesgosAP.class);
    }

	public RiesgosAP crear(final String polizaNumero, final Clientes polizaCliente, final Companias polizaCompania,
			final Date polizaFechaEmision, final Date polizaFechaVigencia, final Date polizaFechaVencimiento,
			final DetalleTipoPagos polizaPago, final double polizaImporteTotal, final float riesgoAPMuerte, 
			final float riesgoAPInvalidez, final float riesgoAPAMF) {
		final RiesgosAP object = new RiesgosAP(polizaNumero, polizaCliente, polizaCompania, polizaFechaEmision,
    ...
    public RiesgosAP renovacion(
			...
			final Date polizaFechaEmision, final Date polizaFechaVigencia, final Date polizaFechaVencimiento,
			final DetalleTipoPagos polizaPago, final double polizaImporteTotal, final float riesgoAPMuerte, 
			final float riesgoAPInvalidez, final float riesgoAPAMF, Polizas riesgoAP) {
        final RiesgosAP object = new RiesgosAP(
